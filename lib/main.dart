...
class _LogisticsHomeScreenState extends State<LogisticsHomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> pages = [
      DashboardHomeTab(currentDate: currentDate, onUpdateNeeded: () => setState(() {})),
      CompletedOrdersTab(),
    ];
...
class DashboardHomeTab extends StatelessWidget {
  ...
  DashboardHomeTab({required this.currentDate, required this.onUpdateNeeded});
              ...
              child: ListView(
                children: [
                  ElevatedButton.icon(
                    ...
                    onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => StoresScreen())).then((_) => onUpdateNeeded()),
                  ),
                  SizedBox(height: 10),
                  ElevatedButton.icon(
                    ...
                    onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => PurchasingScreen())).then((_) => onUpdateNeeded()),
                  ),
                  SizedBox(height: 10),
                  ElevatedButton.icon(
                    ...
                    onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => LabScreen())).then((_) => onUpdateNeeded()),
                  ),
                  SizedBox(height: 10),
                  ElevatedButton.icon(
                    ...
                    onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => FactoriesScreen())).then((_) => onUpdateNeeded()),
                  ),
                  SizedBox(height: 20),
...
class _PurchasingScreenState extends State<PurchasingScreen> {
  ...
  Future<void> _selectDateTime(BuildContext context, OrderModel order, StateSetter setCardState) async {
    ...
    if (pickedDate != null) {
      ...
      if (pickedTime != null) {
        ...
        setState(() {
          order.transportDateTime = '$formattedDate $formattedTime';
        });
        setCardState(() {});
      }
    }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      TextField(
                        ...
                        enabled: canEdit, 
                        decoration: InputDecoration(labelText: 'වාහන අංකය (Vehicle No)', border: OutlineInputBorder()),
                      ),
                      SizedBox(height: 10),
                      Row(
                        ...
                        children: [
                          Expanded(
                            child: Text(
                              'දිනය සහ වේලාව: ${order.transportDateTime}',
                              style: TextStyle(fontWeight: FontWeight.w500, fontSize: 13),
                            ),
                          ),
                          if (canEdit)
                            ElevatedButton.icon(
                              style: ElevatedButton.styleFrom(backgroundColor: Colors.blueGrey, minimumSize: Size(100, 36)),
                              icon: Icon(Icons.calendar_today, size: 16),
                              label: Text('Select Date'),
                              onPressed: () => _selectDateTime(context, order, setCardState),
                            ),
                        ],
                      ),
                      SizedBox(height: 10),
                      if (canEdit)
                        ElevatedButton(
                          ...
                          onPressed: () {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Transport details updated successfully!')));
                          },
                        ),
                    ],
                  ),
...
class _FactoriesScreenState extends State<FactoriesScreen> {
  ...
  void _showNewRequestDialog() {
    ...
    String assignedFactory = (currentUser?.role == UserRole.factory && currentUser?.subCategory.isNotEmpty == true)
    ...
    showDialog(
      context: context,
      builder: (dialogContext) => AlertDialog(
        title: Text('නව ඉල්ලීමක් ඇතුළත් කිරීම ($assignedFactory)'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(
              controller: TextEditingController(text: assignedFactory),
              enabled: false, 
              decoration: InputDecoration(labelText: 'අදාළ කර්මාන්තශාලාව (Factory)', border: OutlineInputBorder()),
            ),
            SizedBox(height: 10),
            TextField(controller: itemController, decoration: InputDecoration(labelText: 'අවශ්‍ය භාණ්ඩය (Item)', border: OutlineInputBorder())),
            SizedBox(height: 10),
            TextField(controller: qtyController, decoration: InputDecoration(labelText: 'ප්‍රමාණය (Quantity)', border: OutlineInputBorder())),
          ],
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(dialogContext), child: Text('Cancel')),
          ElevatedButton(
            ...
            onPressed: () {
              if (itemController.text.isNotEmpty && qtyController.text.isNotEmpty) {
                setState(() {
                ...
                Navigator.pop(dialogContext);
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('ඉල්ලීම සාර්ථකව යවන ලදී!')));
              }
            },
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            Text('කර්මාන්තශාලා 4 හි Stock Sheets:', style: TextStyle(fontWeight: FontWeight.bold)),
            SizedBox(height: 10),
